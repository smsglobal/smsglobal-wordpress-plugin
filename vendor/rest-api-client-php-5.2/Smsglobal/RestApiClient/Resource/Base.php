 ...
 */
abstract class Smsglobal_RestApiClient_Resource_Base
{
    /**
    ...
    public function getResourceName()
    {
        if (null === $this->resourceName) {
            ...
            $this->resourceName = preg_replace('/([A-Z])/', '-$1', $this->resourceName);
    ...
    {
        if (null === $this->id) {
            throw new Smsglobal_RestApiClient_Resource_Exception_NotYetSavedException();
        }
        if (null === $this->resourceUri) {
            $this->resourceUri = sprintf('/v%s/%s/%s/', Smsglobal_RestApiClient_Version::REST_API_VERSION, $this->getResourceName(), $this->id);
        }
